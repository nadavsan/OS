...
// OLD: Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait().

// NEW: This function exits the entire process
//(including all of its kernel threads).
// Therefore, it saves the exit status in the proc and changes its
// state to zombie. The kernel thread state should be changed into
// zombie as well.
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      // TODO: Might need to address when p->state == ZOMBIE and p->state == UNUSED
      if (p->state == RUNNABLE || p->state == RUNNING || p->state == SLEEPING || p->state == USED)
      {
...
// OLD: Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).

// NEW:Find the proc to kill based on its pid and wake up all
// of the proc sleeping kernel threads after setting the process’
//  killed flag value to 1.
int kill(int pid)
{
  struct proc *p;
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    {
      p->killed = 1;
      // not required for the new implementation
      //  if(p->state == SLEEPING){
